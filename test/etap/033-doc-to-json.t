...
-define(json_to_bin(Json), term_to_binary(
    couch_util:json_decode(Json), [{compressed, 1}, {minor_version, 1}])).
-define(json(EJson), couch_util:json_encode(EJson)).
...
test_to_json_success() ->
    Cases = [
        ...
        {
            #doc{deleted = true, body = ?json_to_bin(<<"{\"foo\":\"bar\"}">>)},
        ...
        },
        {
            #doc{
                id = <<"foobar">>,
                body = ?json_to_bin(<<"{\"value\":1,\"data\":\"qwerty\"}">>),
                meta = [
                    {revs_info, 4, [{<<"fin">>, deleted}, {<<"zim">>, missing}]}
                ]
            },
