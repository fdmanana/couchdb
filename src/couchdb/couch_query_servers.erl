...
map_docs(Proc, Docs) ->
    ...
    Results = lists:map(
        fun(Doc) ->
            Json = couch_doc:doc_to_ejson(Doc),
...
validate_doc_update(DDoc, EditDoc, DiskDoc, Ctx, SecObj) ->
    JsonEditDoc = couch_doc:doc_to_ejson(EditDoc, [revs]),
...
json_doc(nil) -> null;
json_doc(Doc) ->
    couch_doc:doc_to_ejson(Doc, [revs]).

filter_view(DDoc, VName, Docs) ->
    JsonDocs = [couch_doc:doc_to_ejson(Doc, [revs]) || Doc <- Docs],
...
teach_ddoc(DDoc, {DDocId, _Rev}=DDocKey, #proc{ddoc_keys=Keys}=Proc) ->
    ...
    JsonDoc = couch_doc:doc_to_ejson(DDoc),
