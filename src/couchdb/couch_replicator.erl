...
do_init(#rep{options = Options, id = {BaseId, Ext}} = Rep) ->
    ...
    couch_task_status:add_task([
        {type, replication}, {source, ?l2b(SourceName)}, {target, ?l2b(TargetName)},
        {continuous, get_value(continuous, Options, false)},
        {revisions_checked, 0}, {missing_revisions_found, 0},
        {docs_read, 0}, {docs_written, 0}, {doc_write_failures, 0},
        {source_seq, SourceCurSeq}, {checkpointed_source_seq, CommittedSeq},
        {progress, 0}, {replication_id, ?l2b(BaseId ++ Ext)}
    ]),
