...
from_sorted_file(EmptyBtree, SortedFileName, DestFd, Options) ->
    Acc0 = #acc{
        btree = EmptyBtree,
        fd = DestFd,
    ...
    SourceFileSize = filelib:file_size(SortedFileName),
    _ = file:advise(SourceFd, 0, SourceFileSize, sequential),
    {ok, Acc2} = try
...
sorted_file_fold(Fd, FileName, Acc) ->
    ...
    {ok, <<Len:32>>} ->
        ...
        {ok, KvBin} ->
            ...
            {ok, Acc2} = fold_copy(Kv, ignored, Acc),
